...
public final class CommandParser {
	...
	private final List<CommandOption> rawOptions = new ArrayList<>();
	...
	public CommandParser(List<CommandOption> options) {
		...
		rawOptions.addAll(options);
		rawOptions.sort(Comparator.comparing(CommandOption::getName));

		command = "";
		usage = new ArrayList<>();
	}

	public CommandParser(String command, List<String> usage, List<CommandOption> options) {
		this(options);
	...
	void printHelp(PrintStream output) {
		output.println();
		output.println(command);
		output.println();
		output.println("Usage:");

		for (String usageCommand : usage) {
			output.print("    ");
			output.println(usageCommand);
		}
		output.println();
		...
		for (CommandOption option : rawOptions) {
			String opt = "    ";

			if (option.getShortName() != CommandOption.NO_SHORTNAME) {
				opt += "-" + option.getShortName() + " ";
			}
			opt += "--";
			opt += option.getName();
			...
			if (option.getArgumentCount() > 0) {
				opt += " <";
				opt += option.getArgument(0);
				opt += ">";
			}
			optionsText.add(opt);
		}

		OptionalInt requiredLength = optionsText.stream()
				.mapToInt(String::length)
				.max();

		for (int i = 0; i < rawOptions.size(); i++) {
			String optionText = optionsText.get(i);
			output.print(optionText);
			output.print(String.join("", Collections.nCopies(requiredLength.getAsInt() + 4 - optionText.length(), " ")));
			output.print(rawOptions.get(i).getDescription());
			output.println(".");
		}
