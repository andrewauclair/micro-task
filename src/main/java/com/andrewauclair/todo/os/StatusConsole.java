...
public class StatusConsole {
	...
	private long applyDateRange(Task task, ZoneId zoneId, LocalDateTime midnight, LocalDateTime nextMidnight) {
		...
		List<TaskTimesFilter.TaskTimeFilterResult> newResults = new ArrayList<>();
...
//		for (Task task : allTasks) {
			...
			if (totalTime > 0) {
				newTasks.add(task);
				newResults.add(new TaskTimesFilter.TaskTimeFilterResult(totalTime, task, tasks.getListForTask(task.id).getFullPath()));
			}
//		}
