...
public class TaskTimesFilter {
	...
	private void buildResults() {
		...
		for (Task task : allTasks) {
			...
			results.add(new TaskTimeFilterResult(totalTime, task, tasks.getListForTask(new ExistingID(tasks.idValidator(), task.ID())).getFullPath()));
		}
	}
	...
	private void applyDateRange(ZoneId zoneId, LocalDateTime midnight, LocalDateTime nextMidnight) {
		...
		for (Task task : allTasks) {
			...
			if (totalTime > 0) {
				newTasks.add(task);
				newResults.add(new TaskTimeFilterResult(totalTime, task, tasks.getListForTask(new ExistingID(tasks.idValidator(), task.ID())).getFullPath()));
			}
		}
